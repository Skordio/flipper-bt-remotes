...
static bool bt_remotes_shortcut_write(Hid* app, FuriString* out_path) {
    furi_string_printf(
        out_path,
        "%s/%s%s",
    ...
    bool           ok  = false;
    if(flipper_format_file_open_always(fff, furi_string_get_cstr(out_path))) {
...
void bt_remotes_scene_profile_shortcut_create_on_enter(void* context) {
    ...
    bool        ok   = bt_remotes_shortcut_write(app, path);

    popup_reset(app->popup);
    if(ok) {
        popup_set_header(app->popup, "Saved!", 64, 10, AlignCenter, AlignTop);
        popup_set_text(
            app->popup, furi_string_get_cstr(path), 64, 28, AlignCenter, AlignTop);
    } else {
