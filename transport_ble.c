...
// All wrappers guard against a NULL ble_hid_profile — bt_remotes_stop_ble NULLs
// the pointer after bt_profile_restore_default so stale input events arriving
// in the brief window after BLE is stopped are silently dropped rather than
// dereferencing a freed profile object.

void hid_hal_keyboard_press(Hid* instance, uint16_t event) {
    furi_assert(instance);
    if(!instance->ble_hid_profile) return;
