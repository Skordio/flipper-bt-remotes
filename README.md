...
- **Launch a profile directly:** In a profile, go to **Settings → Profile Management → Create
  Shortcut**. This writes a `.btremote` file to `apps_data/bt_remotes/launchers/`. Open it from
  the Flipper's File Browser (or pin it to Favorites) and the app boots straight into that
  profile's menu — no Profile Select. Move the `.btremote` anywhere on the SD card; it still works.
