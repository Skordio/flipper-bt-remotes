...
1. **Profile save never wrote `.cfg`** — `bt_remotes_profile_save` only saved the `.keys` file.
   The device name (set via Rename) was silently lost on the next profile activation.

2. **Rename scene called `bt_profile_start` without stopping first** — caused potential
   assertion failure in the BT service. Fixed to use `bt_remotes_stop_ble` / `bt_remotes_start_ble`.

3. **Empty profile name passed validator** — created a hidden `.cfg` file (name starting with
   `.`) that was invisible to the profile list.

4. **`bt_remotes_profile_activate` return value ignored** — BLE was started even on activation
   failure, potentially using a stale identity.

5. **Activation succeeded silently when `.cfg` was missing** — keys were restored but MAC was
   wrong, causing reconnect failures.

6. **Status callback lost after stop/start** — `bt_remotes_start_ble` did not re-register the
   connection status callback, so LED and view connection state broke after any stop/start cycle
   (e.g. entering Settings). Fixed by registering the callback inside `bt_remotes_start_ble`.

7. **New profiles always got a blank BT name** — `bt_remotes_write_cfg` hardcoded `""`.
   Fixed to use `app->default_ble_name`.

8. **Windows 11 "connected but no controls work" after a day** — root cause: auto-save was
   missing, so the bonding keys from the initial pairing were not persisted. On reconnect, the
   Flipper had no keys, triggered `ACI_GAP_BOND_LOST_VSEVT_CODE`, forced a rebond, but the
   Windows HID driver doesn't re-enumerate after a forced rebond. Fixed by auto-saving on
   `BtStatusConnected`.

---

## Session Context Snapshot (May 2026)

The following is a verbatim summary of the two-session development history that produced the
current codebase, preserved here for future agent continuity.

### Session Goals (in order)

1. **Bug audit** — read-only analysis of BLE connection and profile-saving code.
2. **Bug fixes** — implement all 5 bugs found (see "Bugs Fixed" above, bugs 1–5).
3. **Auto-save** — automatically save profile keys+cfg on successful BLE pairing.
4. **Windows 11 HID analysis** — root cause of "connected but no controls work after a day."
5. **Settings menu** — new Settings scene with: Bluetooth Name, Rename Profile, Reset Profile.
6. **Settings accessibility** — Settings reachable from both ProfileSelect and Start; entering
   from Start stops BLE; Back from Settings restarts BLE if a profile was active.
7. **BT name overhaul** — per-profile and default BT name management; `app.cfg` for persistence;
   rename scene context-aware; new profiles inherit default name.
8. **Back-button navigation** — removed "Switch Profile" menu item; Back from Start now stops
   BLE and returns to ProfileSelect.
9. **Settings consolidation** — moved Bluetooth Unpairing, Save Profile, Delete Profile from
   Start menu into Settings menu; added `bt_remotes_profile_clear_pairing` for BLE-safe unpair.

### Key Technical Decisions

- **BLE stop before Settings**: `start_on_event` calls `bt_remotes_stop_ble` before navigating
  to Settings. Settings `on_event` Back handler restarts BLE. This avoids the need for Settings
  sub-scenes to manage BLE state individually.
- **Keys-safe unpair**: `bt_remotes_profile_clear_pairing` (file deletion only) replaces
  `bt_hid_remove_pairing` (BLE API calls) for all unpair operations from inside Settings.
- **Callback consolidation**: Keys storage path + status callback registration moved into
  `bt_remotes_start_ble` so every BLE start cycle is fully self-contained.
- **Immediate profile cfg write on rename**: The rename scene copies `.bt_hid.cfg` to the
  profile's `.cfg` immediately after `bt_hid_save_cfg`, pre-empting the `profile_activate`
  overwrite that happens on Back from Settings.
